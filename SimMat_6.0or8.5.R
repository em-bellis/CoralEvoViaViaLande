...
###function
traject <- function(omega1, omega2, G11, gen2100) {
...
for (l in 1:length(gens.to.run)) {
	for (m in 1:length(omega1.to.run)) {
		for (n in 1:length(h2.to.run)) {
			corr0 <- traject(omega1.to.run[m],omega2.to.run[m],h2.to.run[n],gens.to.run[l])
			U <- cbind(omega1.to.run[m]/P11,omega2.to.run[m]/P22,h2.to.run[n]/P11,gens.to.run[l], corr0)
			df.U <- rbind(df.U, U)
		}
	}
}
